...
def eval3(string:str,flag="noneCompare"):
        ...
        def comparer(s:str):
            if s == "true":
                ...
                for i in range(len(x)):
                    x[i] = x[i].strip()
                    # x[i] = eval3(x[i],flag="noneCompare")
                # print(x)
                ans = []
                typesItem = []
                try:
    ...
    if flag == "noneCompare":
        ...
        # print("b_sharteha:",b_sharteha(string))
...
def tttypeFinder(i : str):
    ...
    if "a" <= i[0] <= "z":
        ...
        print("i:",i)
        # print("re.findall",re.findall(r'\[(.+)\]\[(.+)\]',i))
